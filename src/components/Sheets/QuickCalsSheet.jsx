...
export default function QuickCalsSheet({ onClose, onAdd }) {
  ...
  const handleCommit = () => {
    const result = validateCalories(cals);
    if (result.valid) {
      ...
      <div className="bg-white dark:bg-[#141416] p-6 rounded-t-[32px] border-t border-gray-100 dark:border-[#1f1f23] shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        ...
        <div className="mb-6">
          <input
            ...
            inputMode="decimal"
            value={cals}
            onChange={(e) => { setCals(e.target.value); if (error) setError(null); }}
            onKeyDown={(e) => e.key === 'Enter' && handleCommit()}
            placeholder="0"
            aria-label="Calorie amount"
            aria-invalid={!!error}
          ...
          />
          {error ? (
            <p role="alert" className="text-center text-rose-500 font-bold text-xs mt-3">{error}</p>
          ) : (
        ...
        <button
          ...
          disabled={!cals || !validateCalories(cals).valid}
